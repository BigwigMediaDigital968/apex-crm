...
interface BranchHolidaysTabProps {
    branchId: string;
}
...
export default function BranchHolidaysTab({ branchId }: BranchHolidaysTabProps) {
    ...
    const openEditModal = (holiday: Holiday) => {
        ...
        // Format ISO string back to local YYYY-MM-DD format for date input
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        // Convert string date (YYYY-MM-DD) into ISO DateTime string expected by Zod schema
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    <label className="font-label-md text-xs font-medium text-on-surface-variant">
                        Year:
                    </label>
                    <select
                        value={selectedYear}
                        onChange={(e) => setSelectedYear(Number(e.target.value))}
                        className="rounded-xl border border-outline-variant/40 bg-surface-container-low px-3 py-1.5 font-body-sm text-xs font-bold text-on-surface outline-none focus:border-primary"
                    >
                ...
                <Can permission="holiday:create">
                    <button
                        ...
                        className="flex items-center gap-1.5 rounded-xl bg-primary px-4 py-2 font-label-md text-xs font-bold text-on-primary shadow-sm hover:bg-primary/90 transition-all"
                    >
            ...
            {isLoading ? (
                ...
                <div className="divide-y divide-outline-variant/20 rounded-xl border border-outline-variant/30 overflow-hidden">
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3.5">
                                                <div className="font-label-md text-sm font-bold text-on-surface capitalize">
                                                    {h.name}
                                                </div>
                                                {h.description && (
                                            ...
                                            {/* Date Column */}
                                            <td className="whitespace-nowrap px-4 py-3.5 font-body-sm text-xs font-semibold text-on-surface">
                                                {formattedDate}
                                            </td>
                                            ...
                                            <td className="whitespace-nowrap px-4 py-3.5">
                                                <span
                                                    className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 font-label-sm text-[11px] font-bold ${h.isActive
                                                        ? "bg-emerald-500/10 text-emerald-700"
                                                        : "bg-surface-container-highest text-on-surface-variant/70"
                                                        }`}
                                                >
                                                    <span
                                                        className={`h-1.5 w-1.5 rounded-full ${h.isActive ? "bg-emerald-600" : "bg-on-surface-variant/40"
                                                            }`}
                                                    />
                    ...
                    <div className="w-full max-w-md rounded-2xl bg-surface-container-lowest p-6 shadow-xl space-y-4">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="flex items-center justify-end gap-3 pt-2">
                                ...
                                >
                                    {isSaving ? "Saving..." : editingHoliday ? "Save Changes" : "Create"}
                                </button>
                            </div>
                        </form>
                    </div>
