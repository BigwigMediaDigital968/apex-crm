// src/features/dialer/pages/CallLogsPage.tsx
...
const CallHistoryPage: React.FC = () => {
    ...
    const handleFilterChange = (key: keyof GetCallLogsQueryParams, value: string) => {
        setQueryParams((prev) => ({
            ...prev,
            page: 1, // Reset pagination to page 1 on filter modification
            [key]: value || undefined,
        }));
    };
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-outline-variant/20 pb-4">
                <div>
                    ...
                    <p className="font-body-sm text-xs text-on-surface-variant mt-0.5">
                        Centralized voice history, audio recordings, and telemetry across your entire CRM
                    </p>
                </div>
            ...
            <div className="rounded-2xl border border-outline-variant/30 bg-surface-container-lowest p-4 shadow-sm space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            placeholder="Search phone number, lead..."
                ...
                <div className="flex items-center justify-between border-t border-outline-variant/10 pt-3">
                    <span className="text-[11px] font-semibold text-on-surface-variant">
                        Showing results for selected system parameters
                    </span>
                    <button
                        type="button"
                                        ...
                                        <td className="py-3.5 px-4">
                                            <div className="flex flex-col gap-1 font-mono text-[11px]">
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    <span className="font-semibold text-on-surface">{log.fromNumber || "N/A"}</span>
                                                </div>
                                                <div className="flex items-center gap-1.5">
                                                    <span className="px-1.5 py-0.2 rounded text-[9px] font-sans font-extrabold uppercase bg-primary/10 text-primary">
                                                        To
                                                    </span>
                                                    <span className="font-semibold text-on-surface">{log.toNumber || "N/A"}</span>
                                                </div>
                                            </div>
                                        </td>
                                        {/* Status Badge */}
                ...
                {pagination && pagination.totalPages > 1 && (
                    <div className="flex items-center justify-between px-4 py-3 border-t border-outline-variant/20 bg-surface-container-low">
                        <p className="text-xs text-on-surface-variant">
                            ...
                            <span className="font-bold">{pagination.totalPages}</span> ({pagination.total} entries)
                        </p>
