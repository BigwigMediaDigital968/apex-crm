...
const ExecutiveDashboard: React.FC = () => {
  ...
  // Resolve viewMode consistently for every role:
  // 1. An explicit employee/manager pick always wins.
  // 2. Otherwise use whatever the filter UI already set (BRANCH/TEAM/INDIVIDUAL).
  // 3. Otherwise fall back to a sane per-role default.
  const resolvedViewMode = (() => {
    if (filters.targetUserId) {
      return filters.viewMode === "TEAM" ? "TEAM" : "INDIVIDUAL";
    }
    if (filters.viewMode) return filters.viewMode;
    if (user?.role === "head" || user?.role === "admin") return "BRANCH";
  ...
  // Fetch both managers and employees in scope so PerformanceFilters can
  // populate both the Manager dropdown (HEAD/ADMIN) and Employee dropdown.
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-slate-400 mt-1">
            <span>
              Period: <strong className="text-slate-700 dark:text-slate-300">{formatDate(period.startDate)}</strong> –{" "}
              <strong className="text-slate-700 dark:text-slate-300">{formatDate(period.endDate)}</strong>
            </span>
            {scope?.targetUser && (
              <>
                <span>•</span>
